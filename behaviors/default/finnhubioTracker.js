...
This module manages a list of recent values from a FinnHubIO position
...
FinnHubIOTrackerActor's history is a list of {date<milliseconds>, and amount<dollar>}

*/

class FinnHubIOTrackerActor {
    setup() {
        if (!this.history) {
            this.history = [{ date: 0, amount: 0 }];
        }
        this.listen("AAPL", "onFinnHubIOData");
    }
...
class FinnHubIOTrackerPawn {
    setup() {
        this.lastAmount = 0;
        this.listen("value-changed", "onAAPLChanged");
    ...
    */
    handleUnelected() {
        console.log("FinnHubIO unelected");
    ...
    openSocket() {
        this.closeSocket();
        const apiKey = "cfd8in1r01qj357eshqgcfd8in1r01qj357eshr0";
        const host = "wss://ws.finnhub.io?token="+apiKey;
        const sub_msg = {"type": "subscribe", "symbol": 'AAPL'};
        ...
        this.socket.onmessage = (evt) => {
            ...
            try {
                ...
                last = data.data[data.data.length-1]['p'];
            } catch(e) {
                console.log("invalid data (last: "+last+")");
            }

            if (last !== undefined) {
                this.say("AAPL", { date: Date.now(), amount: +last });
            }
        }
    }
    ...
    latest() {
        return this.actorCall("FinnHubIOTrackerActor", "latest");
    }
    ...
    */
    ...
    onAAPLChanged() {
        ...
        ctx.font = "40px Arial";
        ctx.fillText("AAPL", this.canvas.width - 40, 85);
...
export default {
    modules: [
        {
            name: "FinnHubIOTracker",
            actorBehaviors: [FinnHubIOTrackerActor],
            pawnBehaviors: [FinnHubIOTrackerPawn],
        },
        {
            name: "BarGraph",
